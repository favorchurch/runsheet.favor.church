...
function RunsheetCard({ channel, accessScope, onSelect, onPrefetch }: RunsheetCardProps) {
  ...
  return (
    ...
    >
      <div>
        {/* Top Header Row: Time + Campus Pill inline on left, Location floating on top right */}
        <div className="flex items-start justify-between gap-2">
          <div className="flex items-center gap-2 flex-wrap min-w-0">
            <h3 className="text-xl sm:text-2xl font-extrabold tracking-tight text-slate-900 group-hover:text-blue-600 transition-colors">
              {timeDisplay || titleDisplay || channel.name}
            </h3>
            <span
              className={`inline-flex items-center rounded-lg border px-2.5 py-0.5 text-xs font-bold tracking-wide uppercase shrink-0 ${campusStyle.className}`}
            >
              {campusStyle.label}
            </span>
          </div>
          {titleDisplay && (
            <span className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-slate-50/80 px-2.5 py-0.5 text-xs font-semibold text-slate-700 max-w-[140px] sm:max-w-[170px] truncate shrink-0">
...
function RunsheetCardSkeleton() {
  return (
    <div className="flex flex-col justify-between w-full min-h-[160px] p-5 rounded-2xl border border-slate-200 bg-white shadow-xs animate-pulse">
      <div>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className="h-7 w-20 rounded-lg bg-slate-200" />
            <div className="h-5 w-12 rounded-lg bg-slate-200" />
          </div>
